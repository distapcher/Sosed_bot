...
- `/start` — приветствие
- `/share` — сообщение с описанием и ссылкой для пересылки в группу (карточка t.me)
- `/reset` — очистить память диалога в текущем чате

### Карточка бота в группе (не просто @username)

1. В [@BotFather](https://t.me/BotFather): `/setdescription`, `/setabouttext`, `/setuserpic` — это текст и картинка в превью ссылки `t.me/ваш_бот`.
2. В чате с ботом: `/share` → перешлите сообщение в группу или нажмите «Поделиться в Telegram».

Не копируйте голый `@username` — в **каналах** у `https://t.me/...` часто **нет карточки**, только текст ссылки.

**Для канала** Telegram **не показывает карточку** для `http://`, голого IP и обычно для `t.me/бот` — только текст ссылки.

Рабочий вариант: в боте команда **`/share`** — пришлёт ссылку вида `https://telegra.ph/...` (HTTPS). **Её** вставляйте в канал.

При публикации: **не отключайте превью** (иконка цепочки в редакторе поста).

`http://2.27.25.85/promo` — запасной лендинг; для канала используйте Telegraph из `/share`.
