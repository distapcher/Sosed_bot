...
### Карточка бота в группе или канале

1. В [@BotFather](https://t.me/BotFather): `/setdescription`, `/setabouttext`, `/setuserpic` — текст и аватар в превью `t.me`.
2. В боте: **`/share`** → **перешлите** полученное сообщение (не копируйте только ссылку) — будет превью бота и кнопка «Запустить бота».

Не вставляйте в канал голый `@username` или ссылку вручную — превью надёжнее только через пересылку сообщения от `/share`.
