...
**Для канала** вставляйте promo-ссылку (с заголовком и описанием):

```text
http://2.27.25.85/promo
```

(или ваш домен с HTTPS — так превью стабильнее). В `.env` на сервере: `PUBLIC_PROMO_BASE_URL`.

При публикации в канале: в редакторе поста **не отключайте превью ссылки** (иконка цепочки / «Показать превью»).

Для надёжного превью в Telegram лучше **HTTPS + домен** (Let's Encrypt), чем голый IP.
